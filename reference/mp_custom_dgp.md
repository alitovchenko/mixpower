...
Wraps a user-supplied data-generating function as a validated
`simulate_fun`, so any design the built-in grammar does not cover —
partially nested designs, multi-arm factors, nonlinear time trends,
correlated predictors, or anything else — can still flow through
[`mp_power()`](mp_power.md), [`mp_calibrate()`](mp_calibrate.md),
[`mp_compare_models()`](mp_compare_models.md), and the reporting tools.
The returned function is the escape hatch that keeps the core engine
lean while leaving the design grammar open-ended.

## Usage

``` r
mp_custom_dgp(fun, validate = TRUE)
```
...
- fun:
  ...
  A function `function(scenario, seed = NULL)` returning a `data.frame`.

- validate:

  Validate the output each call (default `TRUE`): a data frame that
  contains all variables in `scenario$formula`.

## Value

A function suitable as a scenario `simulate_fun` (see
[`mp_scenario()`](mp_scenario.md) and the `simulate` argument of
...
[`mp_compare_models()`](mp_compare_models.md) meaningful, read the focal
effect from `scenario$assumptions$fixed_effects` rather than hard-coding
it, so the null (effect = 0) can be generated when those tools set it.

## See also

[`mp_scenario_lme4()`](mp_scenario_lme4.md),
[`mp_scenario()`](mp_scenario.md).
...
#>   power: 0.000
#>   mcse: 0.000
#>   95% CI (clopper-pearson): [0.000, 0.168]
#>   diagnostics:
#>     - fail_rate: 1.000
#>     - singular_rate: 0.000
#>     - type_s: NA
